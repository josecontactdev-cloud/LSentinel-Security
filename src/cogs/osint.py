...
class Osint(commands.Cog):
    ...
    async def whois_lookup(
        ...
        try:
            ...
            embed.add_field(
                name="Domain",
                value=result.domain_name or "Unknown",
                inline=False
            )
            ...
            embed.add_field(
                name="Creation Date",
                value=str(result.creation_date),
                inline=False
            )

            embed.add_field(
                name="Expiration Date",
                value=str(result.expiration_date),
                inline=False
            )

            embed.add_field(
                name="Name Servers",
                value="\n".join(result.name_servers[:5]) if result.name_servers else "Unknown",
                inline=False
            )
